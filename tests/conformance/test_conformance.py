...
class TestTemporalMeta(Base):
    ...
    principals around the subject's actions)."""
    ...
    def _host(self):
        ...
        h.passwd("root", 0).passwd("alice", 1001).passwd("bob", 1002)
        h.enable_execve().enable_network().enable_file_syscalls().watch("/etc", "wa", "etc")
        h.exec(1001, 1001, ["vim", "/etc/hosts"], "/usr/bin/vim", ago(hours=3), comm="vim")
        h.file_change(1001, 0, "chmod", "/etc/hosts", ago(hours=3), euid=0, key="etc")
        h.exec(1002, 1002, ["scp", "data"], "/usr/bin/scp", ago(hours=3, minutes=2),
               comm="scp")
        h.file_change(1002, 0, "creat", "/etc/cron.d/x", ago(hours=3, minutes=1),
                      euid=0, key="etc")
        h.add_user(0, "svc", 1500, ago(hours=5))
        h.write()
        return root

    def _run(self, root, facet, user):
        rc, out = self.cli("--data-root", str(root), "--format", "json",
                           "--facet", facet, "--user", user)
        self.assertEqual(rc, 0, out)
        d = json.loads(out)
        self.assertEqual(d["finding"]["confidence"], "certified")
        self.assertTrue(all(e["records"] for e in d["evidence"]), "provenance lost")
    ...
    def test_tm03_host_changes(self):
        d = self._run(self._host(), "host_changes", "root")
        types = {e["type"] for e in d["finding"]["events"]}
        self.assertIn("file_change", types)
        self.assertIn("account_change", types)
        # host-wide: includes BOTH alice's and bob's changes
        blob = json.dumps(d)
        self.assertIn("/etc/hosts", blob)
        self.assertIn("/etc/cron.d/x", blob)

    def test_tm06_attribution_confidence(self):
        d = self._run(self._host(), "attribution", "alice")
        self.assertIn("high-confidence", d["finding"]["summary"])
        self.assertTrue(any("attribution: high" in n for n in d["finding"]["notes"]))

    def test_tm13_concurrent_other_users(self):
        d = self._run(self._host(), "concurrent", "alice")
        # bob's activity around alice's actions, alice's own excluded
        self.assertTrue(d["finding"]["events"])
        self.assertTrue(all(e.get("actor_name") != "alice"
                            for e in d["finding"]["events"]))


class TestNetflow(Base):
    """Certifies NW-06 (inbound remote peers), NW-14 (transport protocol), and
    NW-04 (byte volume, when conntrack accounting is on) from the conntrack table."""

    def _host(self):
        root = self.make_root()
        h = HostBuilder(root)
        h.passwd("root", 0)
        # inbound tcp from a remote host to local :22, with byte accounting
        h.conntrack("tcp", "203.0.113.9", "10.0.0.2", 51000, 22, nbytes=8192)
        # outbound udp (DNS) with accounting
        h.conntrack("udp", "10.0.0.2", "8.8.8.8", 33000, 53, state="", nbytes=256)
        h.write()
        return root

    def _run(self, root):
        rc, out = self.cli("--data-root", str(root), "--format", "json",
                           "--facet", "netflow", "--user", "root")
        self.assertEqual(rc, 0, out)
        d = json.loads(out)
        self.assertTrue(all(e["records"] for e in d["evidence"]), "provenance lost")
        return d

    def test_nw06_inbound_peers(self):
        d = self._run(self._host())
        inbound = [e for e in d["finding"]["events"]
                   if (e.get("attrs") or {}).get("direction") == "inbound"]
        self.assertTrue(inbound)
        self.assertIn("203.0.113.9", json.dumps(inbound))

    def test_nw14_transport_protocol(self):
        d = self._run(self._host())
        protos = {(e.get("attrs") or {}).get("proto") for e in d["finding"]["events"]}
        self.assertIn("tcp", protos)
        self.assertIn("udp", protos)

    def test_nw04_byte_volume(self):
        d = self._run(self._host())
        self.assertIn("bytes transferred", d["finding"]["summary"])

    def test_nw04_volume_unavailable_without_accounting(self):
        root = self.make_root()
        h = HostBuilder(root)
        h.passwd("root", 0)
        h.conntrack("tcp", "203.0.113.9", "10.0.0.2", 51000, 443)  # no nbytes
        h.write()
        d = self._run(root)
        self.assertIn("byte volume unavailable", d["finding"]["summary"])


class TestFileReads(Base):
    """Certifies FS-12: sensitive files READ (not modified) under a -p r audit
    watch, and that a read is never miscounted as a modification."""

    def test_fs12_sensitive_read(self):
        root = self.make_root()
        h = HostBuilder(root)
        h.passwd("root", 0).passwd("alice", 1001)
        h.enable_execve().enable_file_syscalls().watch("/etc", "r", "shadow-read")
        h.file_read(1001, 0, "/etc/shadow", ago(hours=2), euid=0, key="shadow-read")
        h.file_read(1001, 1001, "/home/bob/.ssh/id_rsa", ago(hours=2), key="ssh-read")
        h.write()
        rc, out = self.cli("--data-root", str(root), "--format", "json",
                           "--facet", "file_access", "--user", "alice")
        self.assertEqual(rc, 0, out)
        d = json.loads(out)
        self.assertTrue(d["finding"]["events"])
        self.assertTrue(all(e["records"] for e in d["evidence"]))
        self.assertIn("SENSITIVE", d["finding"]["summary"])
        self.assertIn("/etc/shadow", json.dumps(d))

    def test_fs12_read_not_counted_as_modification(self):
        # the latent-bug guard: a -p r read must NOT appear in "files modified"
        root = self.make_root()
        h = HostBuilder(root)
        h.passwd("root", 0).passwd("alice", 1001)
        h.enable_execve().enable_file_syscalls().watch("/etc", "r", "shadow-read")
        ...
        h.write()
        f = self.finding(root, "alice", "files")
        paths = {e.attrs.get("path") for e in f.events}
        self.assertNotIn("/etc/shadow", paths)


class TestNonSshAuthAndLockout(Base):
    """Certifies IA-11 (authentication to a non-SSH PAM service) and PV-12 (account
    lockout / faillock) from the general journal, surfaced by the login facet."""

    def _run(self, root, user="alice"):
        rc, out = self.cli("--data-root", str(root), "--format", "json",
                           "--facet", "login", "--user", user)
        self.assertEqual(rc, 0, out)
        d = json.loads(out)
        self.assertTrue(all(e["records"] for e in d["evidence"]), "provenance lost")
        return d

    def test_ia11_non_ssh_auth(self):
        root = self.make_root()
        h = HostBuilder(root)
        h.passwd("root", 0).passwd("alice", 1001)
        h.pam_auth("alice", "cockpit", ago(hours=3))     # non-SSH service
        h.write()
        d = self._run(root)
        self.assertIn("non-SSH", d["finding"]["summary"])
        self.assertIn("cockpit", d["finding"]["summary"])
        self.assertTrue(any((e.get("attrs") or {}).get("kind") == "pam_auth"
                            for e in d["finding"]["events"]))

    def test_ia11_ssh_not_double_reported(self):
        # an sshd PAM line must NOT be re-reported here (the sshd collector owns it)
        root = self.make_root()
        h = HostBuilder(root)
        h.passwd("root", 0).passwd("alice", 1001)
        h.journal_msg("pam_unix(sshd:session): session opened for user alice by (uid=0)",
                      ago(hours=3), ident="sshd")
        h.write()
        d = self._run(root)
        self.assertFalse(any((e.get("attrs") or {}).get("kind") == "pam_auth"
                             for e in d["finding"]["events"]))

    def test_pv12_faillock(self):
        root = self.make_root()
        h = HostBuilder(root)
        h.passwd("root", 0).passwd("alice", 1001)
        h.faillock("alice", ago(hours=2))
        h.write()
        d = self._run(root)
        self.assertIn("lockout", d["finding"]["summary"].lower())
        self.assertTrue(any((e.get("attrs") or {}).get("kind") == "faillock"
                            for e in d["finding"]["events"]))
