...
def sec3_truth_corpus():
    ...
    return (
        ...
        "- **Schema + worked example:** `tests/corpus/example_ground_truth.json` "
        "(operators adapt it to their host).\n"
        ...
        "false-positive claim (OpenPath over-reported).\n"
        "- **Status:** framework complete and runnable; the *populated* corpus (a real "
        "host, ~30 days of usage, human-authored ground truth) is an operator "
        "activity — it is the last mile of trust and is listed as an open item in the "
        "risk register and Go/No-Go below.\n"
    )
...
def sec7_release_checklist():
    checks = [
        ...
        (True, "Host Truth Corpus **framework** shipped + self-tested"),
...
def sec8_go_no_go():
    return (
        ...
        "6. ◻ **RECOMMENDED:** upstream audit/journal capture latency measured on an "
        "instrumented host and within SLA (R3).\n\n"
        "**Current verdict: NO-GO for GA — engineering-complete, pending operational "
        "validation.** Criteria 1–3 are met and machine-verifiable in this repo. "
        "Criteria 4–5 are the true remaining gates and require a real runtime + a "
        "populated corpus, which are operator activities by nature. This is the honest "
