...
| `sources/` | Collectors: `wtmp` (binary), `btmp` (failed logins, shares the utmp parser), `auditd` (streaming), `auth` (syslog auth.log/secure), `journal_sshd`, `packages` (dnf/dpkg), `persistence` (cron/at, systemd units/timers, linger, legacy startup), `authz` (group/sudoers/shadow, authorized_keys, sshd_config), `journald` (general journal: lifecycle + non-SSH auth + faillock), `pkgpolicy` (repos/versionlock/gpg), `shell_history`, `firewall` (nft/iptables), `conntrack`, `netlogs` (dns/fw-drop/proxy/socket), `file_integrity` (FIM diffs), `ip_reputation` (geo/threat feed). |
